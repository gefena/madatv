...
export default function EpisodePageClient({ seriesId, slug }: Props) {
  const { t } = useI18n();
  ...
  if (!episode) {
    return (
      <div className="mx-auto max-w-5xl px-4 py-20 text-center">
        ...
        <Link href={`/series/${seriesId}`} className="mt-4 inline-block text-indigo-600 font-black hover:underline">
          {t("episode.backToAll")}
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-4 sm:py-8 sm:px-6 space-y-6">
      {/* Back link */}
      <Link
